...
import { localizeDocument, type UiLocale } from "./i18n";
import type {
  ActivitySummary,
  AdminServiceSummary,
  AdminUserSummary,
...
type View = "overview" | "users" | "clients" | "pros" | "services" | "jobs" | "payments" | "integrations" | "activity" | "logs";
type Transaction = {
  id: string;
  providerName: string;
  providerType: string;
...
const nav: Array<[View, string, typeof LayoutDashboard]> = [
  ...
  ["logs", "Logs técnicos", ClipboardList],
];
...
export default function AdminPage() {
  ...
  const [systemLogs, setSystemLogs] = useState<SystemLogSummary[]>([]);
  ...
  useEffect(() => { const saved = (localStorage.getItem("fixmy_admin_locale") as UiLocale | null) ?? "es"; setLocale(saved === "fr" || saved === "nl" || saved === "en" || saved === "pt" ? saved : "es"); }, []);
  useEffect(() => { localStorage.setItem("fixmy_admin_locale", locale); return localizeDocument(locale); }, [locale]);
  ...
  async function refresh(silent = false) {
    ...
    try {
      ...
      setUser(me);
      const [d, u, p, s, j, t, i, a, l] = await Promise.all([
        api("/admin/dashboard"), api("/admin/users"), api("/admin/pros"), api("/admin/services"), api("/admin/jobs"),
        api("/payments/admin/transactions"), api("/payments/admin/providers"), api("/admin/activity?limit=150"), api("/admin/system-logs?limit=200"),
      ]);
      setDashboard(d); setUsersData(u); setPros(p); setServices(s); setJobs(j); setPayments(t); setProviders(i); setActivities(a); setSystemLogs((l as { items: SystemLogSummary[] }).items);
  ...
  const visibleLogs = systemLogs.filter((item) => (!lowerSearch || `${item.message} ${item.source} ${item.path ?? ""} ${item.requestId ?? ""}`.toLowerCase().includes(lowerSearch)) && (statusFilter === "ALL" || item.level === statusFilter));
  ...
  return (
    <main className="admin-shell">
      ...
      <section className="admin-main">
        <header className="topbar"><button className="menu-button" onClick={() => setMobileNav(true)}><Menu /></button><div><small>FIX MY CONTROL CENTER</small><h1>{nav.find(([key]) => key === view)?.[1]}</h1></div><div className="top-actions"><label className="admin-locale"><span>◎</span><select value={locale} onChange={event => setLocale(event.target.value as UiLocale)}><option value="es">ES</option><option value="fr">FR</option><option value="nl">NL</option><option value="en">EN</option><option value="pt">PT</option></select></label><span className="live-dot">Sistema operativo</span><button onClick={() => refresh()} disabled={loading}><RefreshCw className={loading ? "spin" : ""} /> {loading ? "Actualizando" : "Actualizar"}</button></div></header>
        ...
        {view === "logs" && <><Toolbar search={search} setSearch={setSearch} placeholder="Buscar mensaje, ruta o request ID" status={statusFilter} setStatus={setStatusFilter} options={["ALL", "ERROR", "WARN", "INFO"]} /><SystemLogs items={visibleLogs} /></>}
...
function SystemLogs({ items }: { items: SystemLogSummary[] }) {
  const errors = items.filter(item => item.level === "ERROR").length;
  const warnings = items.filter(item => item.level === "WARN").length;
  return <><div className="metrics three"><article><span>Eventos visibles</span><strong>{items.length}</strong><small>Registro técnico persistente</small></article><article><span>Errores</span><strong>{errors}</strong><small>Requieren revisión</small></article><article><span>Advertencias</span><strong>{warnings}</strong><small>Fallos de cliente y validación</small></article></div><section className="admin-card data-card"><div className="card-head"><div><small>OBSERVABILIDAD DE TODA LA PLATAFORMA</small><h2>Logs técnicos</h2><p>No se guardan contraseñas, tokens ni credenciales de pago.</p></div></div>{items.length === 0 ? <Empty text="No hay logs con estos filtros."/> : <div className="system-log-list">{items.map(item => <details key={item.id} className={`log-${item.level.toLowerCase()}`}><summary><Status value={item.level}/><span><b>{item.message}</b><small>{item.method ?? "SYSTEM"} {item.path ?? item.source} · {date(item.createdAt)}</small></span><code>{item.statusCode ?? "—"}</code><code>{item.requestId ?? "sin request ID"}</code></summary><div><p><b>Fuente:</b> {item.source}</p>{item.actorId && <p><b>Actor:</b> {item.actorId}</p>}{item.stack && <pre>{item.stack}</pre>}{item.metadata && <pre>{JSON.stringify(item.metadata, null, 2)}</pre>}</div></details>)}</div>}</section></>;
}
